# Configuration file for the Sphinx documentation builder.
#
# This file only contains a selection of the most common options. For a full
# list see the documentation:
# https://www.sphinx-doc.org/en/master/usage/configuration.html

# -- Path setup --------------------------------------------------------------

# If extensions (or modules to document with autodoc) are in another directory,
# add these directories to sys.path here. If the directory is relative to the
# documentation root, use os.path.abspath to make it absolute, like shown here.
#
# import os
# import sys
# sys.path.insert(0, os.path.abspath('.'))

# -- Project information -----------------------------------------------------

project = "skadi"
copyright = "2026, Yasas Wijesekara"
author = "Yasas Wijesekara"
html_title = "skadi"


# -- General configuration ---------------------------------------------------

# Add any Sphinx extension module names here, as strings. They can be
# extensions coming with Sphinx (named 'sphinx.ext.*') or your custom
# ones.
extensions = [
    "sphinx.ext.autodoc",
    "sphinx.ext.extlinks",
    "sphinx.ext.intersphinx",
    "sphinx.ext.mathjax",
    "sphinx.ext.todo",
    "sphinx.ext.viewcode",
    "sphinx_design",
    "sphinx_copybutton",
    "myst_parser",
]
source_suffix = [".rst", ".md"]
myst_enable_extensions = ["colon_fence", "dollarmath", "amsmath"]

# Add any paths that contain templates here, relative to this directory.
templates_path = ["_templates"]

# List of patterns, relative to source directory, that match files and
# directories to ignore when looking for source files.
# This pattern also affects html_static_path and html_extra_path.
exclude_patterns = []


# -- Options for HTML output -------------------------------------------------

# The theme to use for HTML and HTML Help pages.  See the documentation for
# a list of builtin themes.
#
html_theme = "furo"
html_favicon = "_static/figures/skadi.svg"
pygments_style = "catppuccin-latte"
pygments_dark_style = "catppuccin-mocha"
html_theme_options = {
    "sidebar_hide_name": True,
    "navigation_with_keys": True,
    "light_logo": "figures/skadi_logo_light.svg",
    "dark_logo": "figures/skadi_logo_dark.svg",
    "light_css_variables": {
        "font-stack": "'Nunito Sans',system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Fira Sans,Droid Sans,Helvetica Neue,sans-serif",
        "font-stack--monospace": "ui-monospace,Cascadia Mono,Source Code Pro,Segoe UI Mono,Menlo,Monaco,Consolas,Courier New,monospace",
        "admonition-title-font-size": "0.9rem",
        "admonition-font-size": "0.9rem",
        "color-problematic": "#d20f39",
        "color-foreground-primary": "#4c4f69",
        "color-foreground-secondary": "#5c5f77",
        "color-foreground-muted": "#6c6f85",
        "color-foreground-border": "#bcc0cc",
        "color-background-primary": "#eff1f5",
        "color-background-secondary": "#e6e9ef",
        "color-background-hover": "#dce0e8",
        "color-background-hover--transparent": "#dce0e800",
        "color-background-border": "#ccd0da",
        "color-background-item": "#bcc0cc",
        "color-announcement-background": "#000000dd",
        "color-announcement-text": "#dce0e8",
        "color-brand-primary": "#179299",
        "color-brand-content": "#179299",
        "color-highlighted-background": "#df8e1d",
        "color-guilabel-background": "#8caaee80",
        "color-guilabel-border": "#8caaee40",
        "color-api-keyword": "var(--color-foreground-secondary)",
        "color-highlight-on-target": "#eed49f",
        "color-admonition-background": "transparent",
        "color-card-border": "var(--color-background-secondary)",
        "color-card-background": "transparent",
        "color-card-marginals-background": "var(--color-background-hover)",
        "color-code-foreground": "#4c4f69",
        "color-code-background": "#e6e9ef",
    },
    "dark_css_variables": {
        "font-stack": "'Nunito Sans',system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Fira Sans,Droid Sans,Helvetica Neue,sans-serif",
        "font-stack--monospace": "ui-monospace,Cascadia Mono,Source Code Pro,Segoe UI Mono,Menlo,Monaco,Consolas,Courier New,monospace",
        "admonition-title-font-size": "0.9rem",
        "admonition-font-size": "0.9rem",
        "color-problematic": "#f38ba8",
        "color-foreground-primary": "#cdd6f4",
        "color-foreground-secondary": "#a6adc8",
        "color-foreground-muted": "#9399b2",
        "color-foreground-border": "#45475a",
        "color-background-primary": "#1e1e2e",
        "color-background-secondary": "#181825",
        "color-background-hover": "#313244",
        "color-background-hover--transparent": "#31324400",
        "color-background-border": "#313244",
        "color-background-item": "#45475a",
        "color-announcement-background": "#000000dd",
        "color-announcement-text": "#cdd6f4",
        "color-brand-primary": "#94e2d5",
        "color-brand-content": "#94e2d5",
        "color-highlighted-background": "#f9e2af",
        "color-guilabel-background": "#89b4fa30",
        "color-guilabel-border": "#89b4fa20",
        "color-api-keyword": "var(--color-foreground-secondary)",
        "color-highlight-on-target": "#f9e2af30",
        "color-admonition-background": "transparent",
        "color-card-border": "var(--color-background-secondary)",
        "color-card-background": "transparent",
        "color-card-marginals-background": "var(--color-background-hover)",
        "color-code-foreground": "#cdd6f4",
        "color-code-background": "#181825"
    },
    "footer_icons": [
        {
            "name": "GitHub",
            "url": "https://github.com/Yasas1994/skadi/",
            "html": """
                <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"></path>
                </svg>
            """,
            "class": "",
        },
    ],
}

# Add any paths that contain custom static files (such as style sheets) here,
# relative to this directory. They are copied after the builtin static files,
# so a file named "default.css" will overwrite the builtin "default.css".
html_static_path = ["_static"]
html_css_files = ["css/custom.css"]
